...
def add_student():
    ...
    try:
        ...
        cursor = conn.cursor() # Lets you execute SQL commands.
        ...
        e2.delete(0, tk.END)  # Clears the Entry fields after adding.
        e3.delete(0, tk.END)
        e4.delete(0, tk.END)
       
        load_students()  # Reloads the student list to show the new entry

    except mysql.connector.Error as err:
        messagebox.showerror("Database Error", f"Failed to insert student: {err}")
    finally:
        conn.close() # Always close DB connection to free resources.


# Function to update student record
def update_student():
    selected_item = listBox.selection() # Returns selected row ID
...
'''
bind means: "Connect an event to a function.
When the user clicks and releases the left mouse button on the Treeview, 
run the on_treeview_select() function to show the selected row's data in the input fields.
"'''

# Load the student records when the application starts
load_students()

# Start the Tkinter event loop -Without it, the window will open and close instantly. It tells Python to keep the window open and wait for user actions (like typing, clicking, etc.).
root.mainloop()